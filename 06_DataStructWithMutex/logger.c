...
enum {
    ...
    kMaxFileNameLen = 512, /* without null character */
...
/*    strftime(timestamp, size, "%y-%m-%d %H:%M:%S", &calendar);
    ...
    strftime(timestamp, size, "[%Y-%m-%d %H:%M:%S", &calendar);
    sprintf(&timestamp[20], ".%03ld]", (long)time->tv_usec / 1000);//3
    //sprintf(&timestamp[20], ".%06ld]", (long)time->tv_usec);
}
        ...
        long currentTime, long* flushedTime)
{
    ...
    //time
	if ((size = fprintf(fp, "%s[%c]: ", timestamp, levelc)) > 0) {
		totalsize += size;
	}
	...
	if (fmt[strlen(fmt) - 1] == '\n')
	{
        char fmtEx[2 * kMaxFileNameLen]; //max 1024
        strcpy(fmtEx, fmt);
        fmtEx[strlen(fmtEx) - 1] = 0;
		if ((size = vfprintf(fp, fmtEx, arg)) > 0) {
			totalsize += size;
		}
	}
    else if ((size = vfprintf(fp, fmt, arg)) > 0) {
		totalsize += size;
	}

    //file, line, threadID
	if ((size = fprintf(fp, " (%s:%d ID:%ld)", file, line, threadID)) > 0) {
		totalsize += size;
	}
...
void logger_log(LogLevel level, const char* file, int line, const char* fmt, ...)
{
    ...
    {
        /************************************************************************/
        /* The code put here, it's easier to use. Also can move to external.    */
        /************************************************************************/

        //output to console 
        //logger_initConsoleLogger(stderr);

		//out to log_filename, log_file max size, log_file max exist number(log,log1,log2...)
		logger_initFileLogger("./log.txt", 1024 * 1024, 5);

		//log_level < set_level, not print
